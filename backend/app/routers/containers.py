...
from app.schemas import ContainerInfo, ContainerUpdateRequest, ContainerUpdateResult
    ...
    try:
        ...
        if check_updates:
            ...
            for container in containers:
                try:
                    # Get local digest
                    local_digest = await docker_service.get_image_digest(
                        container.image
                    )
                    ...
                    if local_digest:
                        ...
                        (
                            update_available,
                            remote_digest,
                        ) = await registry_service.check_update_available(
                            container.image, local_digest
                        )
...
async def update_container(
    ...
    try:
        ...
        # Update log
        update_log.status = (
            UpdateStatus.SUCCESS if result.success else UpdateStatus.FAILED
        )
...
async def check_all_updates(
    host_id: int, background_tasks: BackgroundTasks, db: AsyncSession = Depends(get_db)
):
    """
...
async def delete_container(
    ...
    try:
        ...
        result = await docker_service.delete_container(
            container_id=container_id, remove_image=remove_image, force=force
        )

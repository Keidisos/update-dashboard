    ...
    async def notify_container_update_available(
        ...
        return await self.send_notification(
            ...
            fields=[
                ...
                {"name": "Current Digest", "value": f"`{current_digest[:16]}...`", "inline": True},
                {"name": "New Digest", "value": f"`{new_digest[:16]}...`", "inline": True},
            ],
    ...
    async def notify_system_updates_available(
        ...
        if packages and len(packages) <= 10:
            ...
            fields.append({
                ...
                "value": ", ".join(packages[:10]) + f" ... and {len(packages) - 10} more",
                "inline": False
            })
    ...
    async def notify_system_updated(
        ...
        if success:
            return await self.send_notification(
                ...
                fields=[
                    {"name": "Host", "value": host_name, "inline": True},
                    {"name": "Packages Updated", "value": str(len(packages_updated)), "inline": True},
                ],
                footer="Update Dashboard"
            )
        else:

...
class DockerService:
    ...
    async def connect(self) -> DockerClient:
        ...
        try:
            if self.host.connection_type == ConnectionType.SSH:
                ...
                # Connect via SSH
                logger.info(
                    f"Connecting to {self.host.hostname}:{self.host.ssh_port} as {self.host.ssh_user}"
                )
                ...
                if exit_code != 0:
                    ...
                    raise DockerException(
                        f"Cannot access Docker on remote host: {error}"
                    )
                ...
                self._client = docker.DockerClient(base_url=base_url, tls=tls_config)
    ...
    async def update_container(
        self, container_id: str, new_image: Optional[str] = None
    ) -> ContainerUpdateResult:
        """
        ...
        try:
            ...
            if hasattr(client, "_exec"):
                ...
                new_container = client.containers.create(
                    image=new_image, name=container_name
                )
    ...
    async def delete_container(
        self, container_id: str, remove_image: bool = True, force: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if remove_image and image_id:
                ...
                image_in_use = any(
                    c.attrs.get("Image") == image_id for c in all_containers
                )
                ...
                if not image_in_use:
                    try:
                        ...
                        removed_items.append(
                            f"Removed image {image_name or image_id[:12]}"
                        )
                    except Exception as e:
                    ...
                    removed_items.append(
                        f"Image {image_name or image_id[:12]} still in use by other containers"
                    )
...
class SSHNetwork:
        ...
        try:
            ...
            if config.networks and config.network_mode not in ("host", "none"):
                ...
                if first_network not in ("bridge", "host", "none"):
                    ...
                    networks_to_connect = {
                        k: v for k, v in config.networks.items() if k != first_network
                    }
            ...
            if networks_to_connect:
                logs.append(
                    f"[6/7] Connecting to {len(networks_to_connect)} additional networks..."
                )
                for net_name, net_config in networks_to_connect.items():
                    try:
                        ...
                        network.connect(
                            new_container,
                            aliases=net_config.get("Aliases"),
                            ipv4_address=net_config.get("IPAMConfig", {}).get(
                                "IPv4Address"
                            )
                        ...
                        logs.append(
                            f"    Warning: Failed to connect to {net_name}: {e}"
                        )
            else:
            ...
            if was_running:
                ...
                if new_container.status != "running":
                    raise Exception(
                        f"Container failed to start: {new_container.status}"
                    )
            else:
                logs.append("    Skipped (original was not running)")
    ...
    async def _rollback_update(
        self, client: DockerClient, original_name: str, logs: List[str]
    ):
        """

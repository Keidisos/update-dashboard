...
class DiscordService:
    ...
    async def send_incident_alert(
        self, incident: SecurityIncident, host_name: str = "Unknown"
    ) -> bool:
        """
        ...
        if not self.should_notify(incident):
            logger.debug(
                f"Skipping Discord notification for {incident.severity.value} incident"
            )
            return False

        try:
            embed = self._create_incident_embed(incident, host_name)
            payload = {"username": "SOC Alert", "embeds": [embed]}
    ...
    def _create_incident_embed(
        self, incident: SecurityIncident, host_name: str
    ) -> dict:
        ...
        # Build fields
        fields = [
            {"name": "🖥️ Host", "value": host_name, "inline": True},
            {
                "name": "⚠️ Severity",
        ...
        if incident.affected_users:
            users = (
                ...
                if isinstance(incident.affected_users, list)
                else []
            )
            if users:
                fields.append(
        ...
        if incident.mitre_techniques:
            techniques = (
                ...
                if isinstance(incident.mitre_techniques, list)
                else []
            )
            if techniques:
                fields.append(
        ...
        embed = {
            ...
            "footer": {"text": f"Incident ID: {incident.id}"},
        }
    ...
    async def send_correlation_alert(
        ...
        try:
            embed = {
                ...
                "footer": {"text": "SOC Correlation Engine"},
            }
